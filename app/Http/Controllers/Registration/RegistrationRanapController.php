...
class RegistrationRanapController extends Controller
{
    public function get(Request $request)
    {
        $registrationData = SphairaRegistration::query();
        $registrationData->where('isDeleted', 0);
        $registrationData->where(DB::raw('SUBSTRING(RegistrationNo, 6, 2)'), '=', 'RI');
        ...
        foreach ($registrations->items() as $registration) {
            ...
            $datas[] = [
                ...
                'location_id'   => $registration->location ? $registration->location->location_id : null,
                'location_name' => $registration->location ? $registration->location->name : null,
